...
const (
	maxBurstBytes                                     = 300 * protocol.DefaultTCPMSS
	renoBeta                       float32            = 0.9999 // Reno backoff factor.
	defaultMinimumCongestionWindow protocol.ByteCount = 200 * protocol.DefaultTCPMSS
)
...
func logMyCubicParams() {
	glog.Infof("quic cubic: beta = %v, betaLastMax = %v\n",
		beta, betaLastMax)
	glog.Infof("quic cubic: maxBurstBytes = %v, defaultMinimumCongestionWindow = %v, renoBeta = %v\n",
		maxBurstBytes, defaultMinimumCongestionWindow, renoBeta)
	glog.Infof("quic cubic: protocol.InitialCongestionWindow: %v, protocol.DefaultMaxCongestionWindow: %v\n",
...
func (c *cubicSender) CanSend(bytesInFlight protocol.ByteCount) bool {
	v := c._CanSend(bytesInFlight)
	if !v {
		c.rejectCount++
	} else {
		c.rejectCount = 0
		c.lastSentPacketTime = time.Now()
	}
	return v
}

func (c *cubicSender) _CanSend(bytesInFlight protocol.ByteCount) bool {
	if !c.noPRR && c.InRecovery() {
...
func (c *cubicSender) MaybeCheckCongestionBug(lastRecievedPacketTime time.Time) {
	if c.rejectCount < 50 {
		return
	}
	now := time.Now()
	// if "lastRecievedPacketTime > now - 5s"
	if lastRecievedPacketTime.Add(5 * time.Second).After(now) {
		glog.Infof("resetting cubic CanSend, lastSentPacketTime = %s, lastRecievedPacketTime = %s", c.lastSentPacketTime.String(), lastRecievedPacketTime.String())
		c.rejectCount = 0
		c.lastSentPacketTime = now
