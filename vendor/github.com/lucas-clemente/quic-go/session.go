...
type session struct {
	...
	logger                 utils.Logger
	lastRecievedPacketTime time.Time
}
...
runLoop:
	...
	s.closed.Set(true)
	s.logger.Infof("Connection src %s dst %s closed.", s.srcConnID, s.destConnID)
...
func (s *session) maybeHandleConnMigration(packetAddr net.Addr) {
	// Supports roaming
	if !s.isClient {
		// glog.Infof("connection %s: old %s new %s", s.destConnID, s.conn.RemoteAddr().String(), packetAddr.String())
		// TODO: perf: do not check on every packet. only check a subset of it
		if oldAddr := s.conn.SetCurrentRemoteAddr(packetAddr); oldAddr != nil {
			glog.Infof("connection %s: Address migrated from %s to %s", s.destConnID, oldAddr.String(), packetAddr.String())
			s.sentPacketHandler.OnConnectionMigration()
		}
	}
}
...
func (s *session) handleSinglePacket(p *receivedPacket, hdr *wire.Header) bool /* was the packet successfully processed */ {
	...
	s.maybeHandleConnMigration(p.remoteAddr)
	s.lastRecievedPacketTime = time.Now()
...
func (s *session) closeLocal(e error) {
	s.closeOnce.Do(func() {
		...
		glog.Infof("closing session %v with error: %v", s.RemoteAddr(), e)
	})
}
...
sendLoop:
	for {
		...
		case ackhandler.SendAck:
			// There is a bug as 2019-07-13 that the sender could get
			// congestion limited forever.
			s.sentPacketHandler.MaybeCheckCongestionBug(s.lastRecievedPacketTime)
