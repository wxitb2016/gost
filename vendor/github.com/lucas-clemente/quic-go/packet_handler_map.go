...
func (h *packetHandlerMap) Add(id protocol.ConnectionID, handler packetHandler) {
	if session, ok := handler.(*session); ok {
		h.closeAllConflictingSessions(session.destConnID)
	}
	h.mutex.Lock()
...
func (h *packetHandlerMap) closeAllConflictingSessions(newID protocol.ConnectionID) {
	h.mutex.Lock()
	// glog.Infof("newID: %v\n", []byte(newID))
	// glog.Infof("closeAllConflictingSessions: total %d sessions\n", len(h.handlers))
	// for _, handler := range h.handlers {
	// 	if handler == nil {
	// 		continue
	// 	}
	// 	glog.Infof(">> handler: %v\n", []byte(handler.(*session).destConnID))
	// }
	...
	for _, handler := range h.handlers {
		...
		id := []byte(handler.(*session).destConnID)
		if isSameConnectionIDWithDifferentEpoch(protocol.ConnectionID(id), newID) {
			glog.Infof("closeAllConflictingSessions: found 1\n")
			wg.Add(1)
			go func(handler packetHandler) {
				// session.Close() blocks until the CONNECTION_CLOSE has been sent and the run-loop has stopped
				_ = handler.Close()
	...
	// session.Close() would call h.Remote finally, which needs the
	// lock. So we unlock the mutex before wg.wait to avoid a
	// deadlock.
	h.mutex.Unlock()
	wg.Wait()
	return
